...
do
    ...
    mpiexec -n 16 --allow-run-as-root python classifier_sample.py \
...
#      --use_ddim False --timestep_respacing 250
    ...
    mpiexec -n 16 --allow-run-as-root python classifier_sample.py \
      $DIFFUSION_FLAGS $MODEL_FLAGS $CLASSIFIER_FLAGS $SAMPLE_FLAGS \
      --log_root ${logdir} \
      --save_name ${save_name}/predict/model${t}_scale15.0 \
      --classifier_scale 15.0 --num_samples 10000 --batch_size 32 \
      --model_path ${pretrain_model}/256x256_diffusion_uncond.pt \
      --classifier_path ${logdir}/${save_name}/model${t}.pt \
      --gpus 0,1,2,3,4,5,6,7
#      --use_entropy_scale True
#      --use_ddim False --timestep_respacing 250
    ...
    mpiexec -n 16 --allow-run-as-root python classifier_sample.py \
      $DIFFUSION_FLAGS $MODEL_FLAGS $CLASSIFIER_FLAGS $SAMPLE_FLAGS \
      --log_root ${logdir} \
      --save_name ${save_name}/predict/model${t}_scale15.0_entropyScale \
      --classifier_scale 15.0 --num_samples 10000 --batch_size 32 \
      --model_path ${pretrain_model}/256x256_diffusion_uncond.pt \
      --classifier_path ${logdir}/${save_name}/model${t}.pt \
      --use_entropy_scale True \
