...
class IPR():
    def __init__(self, batch_size=50, k=3, num_samples=10000, model=None):
        ...
        if model is None:
            ...
            self.inception = models.inception_v3(pretrained=True).cuda().eval()
            # print(self.vgg16)
            print('vgg16 and inception done')
        else:
            self.vgg16 = model
    ...
    def extract_features(self, images):
        ...
        for bi in trange(num_batches, desc=desc):
            ...
            before_fc = before_fc.contiguous().view(-1, 7 * 7 * 512)
...
if __name__ == '__main__':
    ...
    print('loading')
    fake_data = np.load(args.path_fake)['arr_0']
    fake_data = fake_data.transpose(0, 3, 1, 2)[:5000]
    
    real_data = np.load(args.path_real)['arr_0']
    real_data = real_data.transpose(0, 3, 1, 2)[:5000]
    ...
    with torch.no_grad():
        # real
        ipr.compute_manifold_ref(real_data)
        ...
        # fake
        precision, recall = ipr.precision_and_recall(fake_data)
