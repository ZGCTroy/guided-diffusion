...
def run_wrapper(submit_config: SubmitConfig) -> None:
    ...
    if is_local:
        logger = util.Logger(file_name=os.path.join(submit_config.run_dir, "groundth_normal_inference_log.txt"), file_mode="w", should_flush=True)
    ...
    try:
        ...
        if is_local:
            ...
            log_src = os.path.join(submit_config.run_dir, "groundth_normal_inference_log.txt")
